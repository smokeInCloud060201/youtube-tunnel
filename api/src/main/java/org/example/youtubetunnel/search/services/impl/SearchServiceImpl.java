...
//        SearchVideoResponseDTO searchVideoResponseDTO = webClientUtil.get(VIDEO_URL, queryParams, Collections.emptyMap(), SearchVideoResponseDTO.class).join();
    ...
    private VideoDTO mapItemToVideoDTO(SearchVideoResponseDTO.Item item) {
        ...
        CompletableFuture<String> defaultThumb = webClientUtil.get(thumbnails.getDefaultThumbnail().getUrl(), byte[].class)
                .thenApply(bytes -> Base64.getEncoder().encodeToString(bytes));

        CompletableFuture<String> mediumThumb = webClientUtil.get(thumbnails.getMedium().getUrl(), byte[].class)
                .thenApply(bytes -> Base64.getEncoder().encodeToString(bytes));

        CompletableFuture<String> highThumb = webClientUtil.get(thumbnails.getHigh().getUrl(), byte[].class)
                .thenApply(bytes -> Base64.getEncoder().encodeToString(bytes));
    ...
    public SearchVideoResponseDTO buildResponse() {
        ...
        snippet1.setDescription("Một trong những khúc ca xúc động nhất album. Tình yêu quốc kỳ được thắp sáng – lá cờ theo chân người Việt ra khắp năm châu, ...");
        ...
        snippet2.setDescription("vtvshows #haanhtuan Nhà tôi có một lá cờ - Hà Anh Tuấn | Cầu truyền hình Thời cơ Vàng #VTVSHOWS là kênh giải trí số của Đài ...");
        ...
        snippet3.setDescription("NHÀ TÔI CÓ TREO MỘT LÁ CỜ | Ca sĩ Hà Anh Tuấn | Cầu truyền hình THỜI CƠ VÀNG NHÀ TÔI CÓ TREO MỘT LÁ CỜ Sáng tác ...");
        ...
        snippet4.setTitle("HÀ ANH TUẤN NGÂN VANG CA KHÚC “NHÀ TÔI CÓ TREO MỘT LÁ CỜ” TRONG CHƯƠNG TRÌNH \"THỜI CƠ VÀNG\"");
        snippet4.setDescription("Nhà tôi có treo một lá cờ” – ca khúc đầy tự hào được ca sĩ Hà Anh Tuấn thể hiện trong chương trình Thời Cơ Vàng, giữa không ...");
        ...
        snippet5.setDescription("rangrovietnam #VTVdigital Nhà tôi có treo một lá cờ | Hà Anh Tuấn – DTAP | Cầu Truyền hình trực tiếp Thời cơ vàng \"Rạng rỡ ...");
