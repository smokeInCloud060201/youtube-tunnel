...
public class WebClientUtil {
    ...
    private static final Retry RETRY_POLICY = Retry.backoff(3, Duration.ofMillis(500))
            ...
            .doBeforeRetry(retrySignal -> log.info("Retry attempt {} due to {}",
    ...
    private <R> CompletableFuture<R> execute(Mono<R> mono, String uri) {
        return mono
                ...
                .onErrorMap(throwable -> {
                    if (throwable instanceof ApplicationException) return throwable;
    ...
    public <R> CompletableFuture<R> get(String uri, Map<String, String> queryParams, Map<String, String> headers, Class<R> responseType) {
        WebClient.RequestHeadersSpec<?> request = applyHeaders(
    ...
    public <T, R> CompletableFuture<R> post(String uri, T requestBody, Map<String, String> headers, Class<R> responseType) {
    ...
    public <R> CompletableFuture<R> postForm(String uri, MultiValueMap<String, Object> formData, Map<String, String> headers, Class<R> responseType) {
        WebClient.RequestBodySpec bodySpec = webClient.post()
                .uri(uri)
                .contentType(MediaType.MULTIPART_FORM_DATA);

        WebClient.RequestHeadersSpec<?> request = applyHeaders(bodySpec.body(BodyInserters.fromMultipartData(formData)), headers);

        return execute(handleErrors(request).bodyToMono(responseType), uri);
    }

    public <R> CompletableFuture<R> postForm(String uri, MultiValueMap<String, Object> formData, Class<R> responseType) {
